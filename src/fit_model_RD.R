#This code is based on the function fit_model in the ladybird package.
...
fit_model <- function(
  ...
  assert_that(
    has_name(all_prediction, "iyear"), has_name(all_prediction, "iyear2"),
  ...
  if (is_secondary) {
    ...
    assert_that(
      ...
      has_name(base_prediction, "cyear"), has_name(all_prediction, "cyear")
    )
  }
  ...
  index_estimate <- inla.stack.index(stack3, "all_prediction")$data
  m3$summary.fitted.values[index_estimate, ] %>%
    select(.data$mean, median = 4, lcl = 3, ucl = 5) %>%
    as_tibble() %>%
    bind_cols(
      m3$summary.linear.predictor[index_estimate, ] %>%
        select(lp_mean = .data$mean, lp_median = 4, lp_lcl = 3, lp_ucl = 5),
      all_prediction %>%
        select(.data$year, .data$location)
    ) -> predictions_all
