...
data %>% group_by(BekkenNaam, jaar) %>%
  summarise(N = n()) %>% 
  tidyr::complete(BekkenNaam, jaar) %>%
  ungroup() %>%
...
data %>% group_by(jaar) %>% st_drop_geometry() %>%
  summarize(Resistentie = mean(Resistent),
         M1 = mean(MutatieM1),
         M2 = mean(MutatieM2),
         M3 = mean(MutatieM3)) %>%    ### Gaat rare zaken geven door NA gelijk te stellen aan 0. Na is Na en niet 0. missing data niet gelijk zetten aan 0. Gaat ook rare zaken geven in het model
